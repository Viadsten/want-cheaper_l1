...
document.getElementById("header__h2").innerHTML = "Обучение торговле<br>на финансовых рынках";
document.getElementById("header__title").innerHTML = "Начни изучать уже сегодня, как правильно<br>управлять капиталом";
document.getElementById("radio-learn").classList.add('radio-label--active');
document.getElementById("subscribe-btn").style.display = "none";

var cource_btn = document.getElementsByName('course-radio');
var course_base = document.getElementById("course-base");
var course_fibpro = document.getElementById("course-fibpro");
var course_trade = document.getElementById("course-trade");

function check_trade()
{
    for (var j = 0; j < cource_btn.length; j++) {
        if (cource_btn[j].type == "radio" && cource_btn[j].checked) {

          if (j == 0){
            course_base.style.display = "block";
            course_fibpro.style.display = "none";
            course_trade.style.display = "none";
          }
          else if (j == 1){
            course_base.style.display = "none";
            course_fibpro.style.display = "block";
            course_trade.style.display = "none";
          }
          else {
            course_base.style.display = "none";
            course_fibpro.style.display = "none";
            course_trade.style.display = "block";
          }
        }
    }
}
...
cource_btn.onclick = check_trade();
